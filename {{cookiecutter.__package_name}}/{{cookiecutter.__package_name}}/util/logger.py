...
from {{cookiecutter.__package_name}}.util.exceptions import {{cookiecutter.instr_name}}Warning
...
class MyLogger(AstropyLogger):
    ...
    def _showwarning(self, *args, **kwargs):
        ...
        if not isinstance(args[0], {{cookiecutter.instr_name}}Warning):
        ...
        # be displayed.
        if type(warning) not in ({{cookiecutter.instr_name}}Warning,):
...
def _init_log(config=None):
    ...
    try:
        log = logging.getLogger("{{cookiecutter.__package_name}}")
