...
from {{cookiecutter.__package_name}}.util.exceptions import warn_user
...
if not os.getenv("LAMBDA_ENVIRONMENT"):
    ...
    dirs = AppDirs("{{cookiecutter.__package_name}}", "{{cookiecutter.__package_name}}")
...
def load_config():
    ...
    if not config.has_option("general", "working_dir"):
        config.set("general", "working_dir", str(Path.home() / "{{cookiecutter.__package_name}}"))
...
def _find_config_files():
    ...
    module_dir = Path({{cookiecutter.__package_name}}.__file__).parent
...
def get_and_create_download_dir():
    ...
    """
    download_dir = os.environ.get("{{cookiecutter.instr_name}}_DOWNLOADDIR")
    if download_dir:
        return download_dir
...
def get_and_create_sample_dir():
    ...
    """
    sample_dir = (
        Path({{cookiecutter.__package_name}}.config.get("downloads", "sample_dir")).expanduser().resolve()
    )
    ...
    return {{cookiecutter.__package_name}}.config.get("downloads", "sample_dir")
...
def print_config():
    ...
    print("\nCONFIGURATION:")
    for section in {{cookiecutter.__package_name}}.config.sections():
        print(f"  [{section}]")
        for option in {{cookiecutter.__package_name}}.config.options(section):
            print(f"  {option} = {{cookiecutter.__package_name}}.config.get(section, option)")
        print("")
...
def _get_user_configdir():
    ...
    "{{cookiecutter.instr_name}}_CONFIGDIR" environment variable.
    """
    configdir = os.environ.get("{{cookiecutter.instr_name}}_CONFIGDIR", CONFIG_DIR)

    if not _is_writable_dir(configdir):
        raise RuntimeError(f'Could not write to {{cookiecutter.instr_name}}_CONFIGDIR="{configdir}"')
    return configdir
...
def copy_default_config(overwrite=False):
    ...
    """
    config_filename = "configrc"
    config_file = Path({{cookiecutter.__package_name}}.__file__).parent / "data" / config_filename
